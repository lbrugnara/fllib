...
void parse_derived_normalization_properties(FlVector *data)
{
    ...
    FlArray *tmpbuffer = fl_io_file_read_all_bytes("resources/unicode/DerivedNormalizationProps-9.0.0.txt");
    char *buffer = NULL;
...
{
    FlArray *tmpbuffer = fl_io_file_read_all_bytes("resources/unicode/UnicodeData-9.0.0.txt");
...
void create_unicode_database_file(FlVector *data)
{
    ...
    FILE *size_file_fd = fl_io_file_open("src/text/UnicodeDataDbLength.h", "w");
    fprintf(size_file_fd, "#ifndef FL_UNICODE_DB_LENGTH_H\n");
    fprintf(size_file_fd, "#define FL_UNICODE_DB_LENGTH_H\n\n");
    fprintf(size_file_fd, "#include <stddef.h>\n\n");
    fprintf(size_file_fd, "size_t UnicodeDbLength = %zu;\n\n", data_length);
    fprintf(size_file_fd, "#endif /* FL_UNICODE_DB_LENGTH_H */\n");
    fl_io_file_close(size_file_fd);

    FILE * outfd = fl_io_file_open("src/text/UnicodeDataDb.h", "w");
    ...
    for (size_t i=0; i < data_length; i++)
    {
        ...
        // NOTE: IMPORTANT! We don't use designated initializers because MSVC doesn't support
        // it (hopefully "yet"...)
        ...
        if (ud->bidi_class)
            ...
            fprintf(outfd, ", /* .bidi_class = */ NULL");

        // const uint32_t* decomposition_mapping;
        if (ud->decomposition_mapping)
            fprintf(outfd, ", /* .decomposition_mapping = */ (const uint32_t[]){ %s, UINT32_MAX }", ud->decomposition_mapping);
        else
            fprintf(outfd, ", /* .decomposition_mapping = */ NULL");

        // double numerical_value_3;
        ...
        if (ud->simple_uppercase_mapping)
            ...
            fprintf(outfd, ", /* .simple_uppercase_mapping = */ 0x0");
        ...
        if (ud->simple_lowercase_mapping)
            ...
            fprintf(outfd, ", /* .simple_lowercase_mapping = */ 0x0");
        ...
        if (ud->full_composition_exclusion)
            ...
            fprintf(outfd, ", /* .full_composition_exclusion = */ false");
        
        // FlByte canonical_combining_class;
        if (ud->canonical_combining_class)
            fprintf(outfd, ", /* .canonical_combining_class = */ %s",     ud->canonical_combining_class);
        else
            fprintf(outfd, ", /* .canonical_combining_class = */ 0");
