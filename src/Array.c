...
FlArray* fl_array_combine(FlArray *dest_array, const FlArray *const src_array)
{
    ...
    for (size_t i = 0; orig_length < new_length; orig_length++, i++) {
        memcpy((FlByte*) dest_array + (orig_length * dest_header->element_size),
               (FlByte*) src_array + (i * src_header->element_size),
               dest_header->element_size);
    }
...
{
    return FL_ARRAY_HEADER(array)->length;
}

size_t fl_array_element_size(const FlArray *const array)
{
    return FL_ARRAY_HEADER(array)->element_size;
}
