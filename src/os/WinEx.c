...
void fl_winex_message_get(DWORD exceptionCode, char *destmsg, size_t maxlength)
{
    ...
    switch (exceptionCode) 
    {
        case EXCEPTION_ACCESS_VIOLATION:
            msg = fl_cstring_dup("[EXCEPTION_ACCESS_VIOLATION] The thread tried to read from or write to a virtual address for which it does not have the appropriate access.");
            break;
        case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
            msg = fl_cstring_dup("[EXCEPTION_ARRAY_BOUNDS_EXCEEDED] The thread tried to access an array element that is out of bounds and the underlying hardware supports bounds checking.");
            break;
        case EXCEPTION_BREAKPOINT:
            msg = fl_cstring_dup("[EXCEPTION_BREAKPOINT] A breakpoint was encountered.");
            break;
        case EXCEPTION_DATATYPE_MISALIGNMENT:
            msg = fl_cstring_dup("[EXCEPTION_DATATYPE_MISALIGNMENT] The thread tried to read or write data that is misaligned on hardware that does not provide alignment. For example, 16-bit values must be aligned on 2-byte boundaries; 32-bit values on 4-byte boundaries, and so on.");
            break;
        case EXCEPTION_FLT_DENORMAL_OPERAND:
            msg = fl_cstring_dup("[EXCEPTION_FLT_DENORMAL_OPERAND] One of the operands in a floating-point operation is denormal. A denormal value is one that is too small to represent as a standard floating-point value.");
            break;
        case EXCEPTION_FLT_DIVIDE_BY_ZERO:
            msg = fl_cstring_dup("[EXCEPTION_FLT_DIVIDE_BY_ZERO] The thread tried to divide a floating-point value by a floating-point divisor of zero.");
            break;
        case EXCEPTION_FLT_INEXACT_RESULT:
            msg = fl_cstring_dup("[EXCEPTION_FLT_INEXACT_RESULT] The result of a floating-point operation cannot be represented exactly as a decimal fraction.");
            break;
        case EXCEPTION_FLT_INVALID_OPERATION:
            msg = fl_cstring_dup("[EXCEPTION_FLT_INVALID_OPERATION] This exception represents any floating-point exception not included in this list.");
            break;
        case EXCEPTION_FLT_OVERFLOW:
            msg = fl_cstring_dup("[EXCEPTION_FLT_OVERFLOW] The exponent of a floating-point operation is greater than the magnitude allowed by the corresponding type.");
            break;
        case EXCEPTION_FLT_STACK_CHECK:
            msg = fl_cstring_dup("[EXCEPTION_FLT_STACK_CHECK] The stack overflowed or underflowed as the result of a floating-point operation.");
            break;
        case EXCEPTION_FLT_UNDERFLOW:
            msg = fl_cstring_dup("[EXCEPTION_FLT_UNDERFLOW] The exponent of a floating-point operation is less than the magnitude allowed by the corresponding type.");
            break;
        case EXCEPTION_GUARD_PAGE:
            msg = fl_cstring_dup("[EXCEPTION_GUARD_PAGE] The thread accessed memory allocated with the PAGE_GUARD modifier.");
            break;
        case EXCEPTION_ILLEGAL_INSTRUCTION:
            msg = fl_cstring_dup("[EXCEPTION_ILLEGAL_INSTRUCTION] The thread tried to execute an invalid instruction.");
            break;
        case EXCEPTION_IN_PAGE_ERROR:
            msg = fl_cstring_dup("[EXCEPTION_IN_PAGE_ERROR] The thread tried to access a page that was not present, and the system was unable to load the page. For example, this exception might occur if a network connection is lost while running a program over the network.");
            break;
        case EXCEPTION_INT_DIVIDE_BY_ZERO:
            msg = fl_cstring_dup("[EXCEPTION_INT_DIVIDE_BY_ZERO] The thread tried to divide an integer value by an integer divisor of zero.");
            break;
        case EXCEPTION_INT_OVERFLOW:
            msg = fl_cstring_dup("[EXCEPTION_INT_OVERFLOW] The result of an integer operation caused a carry out of the most significant bit of the result.");
            break;
        case EXCEPTION_INVALID_DISPOSITION:
            msg = fl_cstring_dup("[EXCEPTION_INVALID_DISPOSITION] An exception handler returned an invalid disposition to the exception dispatcher. Programmers using a high-level language such as C should never encounter this exception.");
            break;
        case EXCEPTION_INVALID_HANDLE:
            msg = fl_cstring_dup("[EXCEPTION_INVALID_HANDLE] The thread used a handle to a kernel object that was invalid (probably because it had been closed.)");
            break;
        case EXCEPTION_NONCONTINUABLE_EXCEPTION:
            msg = fl_cstring_dup("[EXCEPTION_NONCONTINUABLE_EXCEPTION] The thread tried to continue execution after a noncontinuable exception occurred.");
            break;
        case EXCEPTION_PRIV_INSTRUCTION:
            msg = fl_cstring_dup("[EXCEPTION_PRIV_INSTRUCTION] The thread tried to execute an instruction whose operation is not allowed in the current machine mode.");
            break;
        case EXCEPTION_SINGLE_STEP:
            msg = fl_cstring_dup("[EXCEPTION_SINGLE_STEP] A trace trap or other single-instruction mechanism signaled that one instruction has been executed.");
            break;
        case EXCEPTION_STACK_OVERFLOW:
            msg = fl_cstring_dup("[EXCEPTION_STACK_OVERFLOW] The thread used up its stack.");
            break;
        // Status
        case STATUS_SEGMENT_NOTIFICATION:
            msg = fl_cstring_dup("[STATUS_SEGMENT_NOTIFICATION]");
            break;
        case STATUS_NO_MEMORY:
            msg = fl_cstring_dup("[STATUS_NO_MEMORY] Not enough virtual memory or paging file quota is available to complete the specified operation.");
            break;
        case STATUS_CONTROL_C_EXIT:
            msg = fl_cstring_dup("[STATUS_CONTROL_C_EXIT]");
            break;
        default:
            msg = fl_cstring_vdup("Unknown error: code %lu", exceptionCode);
            break;
    }
    if (msg == NULL)
        return;
